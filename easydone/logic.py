...
def normalize_due_date(value: str | None, today: date | None = None) -> str | None:
    ...
    if parsed.strftime("%Y-%m-%d") != normalized:
        raise ValueError(f"Invalid due date ({value}); use YYYY-MM-DD, Tomorrow, +/-N, or Clear")
    return normalized
...
class TasksManager:
        ...
        *,
        ...
        if new_prior is not None:
            if new_prior not in SUPPORTED_PRIORITIES:
                raise ValueError(
                    f"Attempting to update task: {id} with invalid priority: {new_prior}"
                )
        ...
        updated_tags = [tag for tag in updated_tags if tag not in normalized_remove_tags]
    ...
    def update_many(self, ids: list[str], **changes: object) -> bool:
        ...
        try:
            updated = False
            for id in unique_ids:
                # '|=' is called boolean acumulation
                # it will set updated to True if any of the updates return True
                updated |= self.update(
                    id,
                    # manually cast the values to Any to avoid type checking issues
                    # since we are passing **changes which is a dict of str to object
    ...
    def _task_id(self) -> str:
        ...
        the number of digits is determined by self.ID_SIZE.
        """
        id = None
